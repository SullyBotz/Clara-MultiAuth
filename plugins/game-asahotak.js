...
Bonus: ${poin} XP
    `.trim()
    conn.asahotak[id] = [
        await conn.sendMessage(m.chat, caption, wm, m),
        json, poin,
        setTimeout(async () => {
            if (conn.asahotak[id]) await conn.sendMessage(m.chat, `Waktu Habis!\nJawabannya Adalah *${json.jawaban}*`, wm, conn.asahotak[id][0])
